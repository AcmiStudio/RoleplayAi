"""
client.py – Клиент RPG с поддержкой цвета игрока
"""
import asyncio
import json
...
class RPGGameClient:
    ...
    async def connect_to_server(self, uri):
        try:
            ...
            self.ui.print_error("Не удалось подключиться к серверу")
    ...
    async def process_server_message(self, data):
        ...
        if msg_type == "room_info":
            ...
            # Получаем цвет от сервера, если он есть
            color_name = data.get("player_color", "WHITE")
